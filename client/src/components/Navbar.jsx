...
const Navbar = () => {
  ...
  <motion.div
    className="absolute right-0 z-10 hidden py-3 border border-red-200 rounded shadow-sm group-hover:block group-focus-within:block w-36 bg-slate-100"
    ...
    <div className="flex flex-col gap-2 px-5 py-3 text-gray-500 rounded cursor-pointer">
      ...
      <p className="flex gap-1 cursor-pointer hover:text-black">
        <CiLogout /> Logout
            ...
            {/* Mobile Menu Overlay */}
            <motion.div
                className={`fixed top-0 right-0 bottom-0 z-40 bg-slate-100 transition-all ${visible ? "w-full" : "w-0"}`}
                initial={{ width: 0 }}
            ...
            >
                <div className="flex flex-col h-full text-gray-600">
                    {/* Close Menu Button */}
                    <div onClick={() => setVisible(false)} className="flex items-center gap-4 p-3 px-5">
                        <MdClose className="h-4 text-2xl rotate-180 bg-gray-100 rounded cursor-pointer" />
                    </div>
                    {/* Mobile Links */}
                    <NavLink onClick={() => setVisible(false)} className={"py-2 p-6 border"} to={"/"}>Home</NavLink>
                    <NavLink onClick={() => setVisible(false)} className={"py-2 p-6 border"} to={"/about"}>About</NavLink>
                    <NavLink onClick={() => setVisible(false)} className={"py-2 p-6 border"} to={"/shops"}>Shops</NavLink>
                    <NavLink onClick={() => setVisible(false)} className={"py-2 p-6 border"} to={"/contact"}>Contact</NavLink>
                </div>
            </motion.div>
        </div>
    );
